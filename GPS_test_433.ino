...
void turnOffLEDs(){
    digitalWrite(red_LED, LOW);                                    // Turn the red LED off by making the voltage HIGH
    digitalWrite(green_LED, LOW);                                  // Turn the green LED off by making the voltage HIGH
    green_led_on = false;
}
...
void blinkLEDTracking(){
    ...
    if (green_led_on){
        ...
        digitalWrite(green_LED, HIGH);                               // Turn the green LED on by making the voltage LOW
    }
...
void get_coords(){
    ...
    if(newData && gps.location.isValid() && gps.altitude.isValid() && gps.hdop.isValid() && (gps.location.age() < 1000)){
        ...
        if (!newData) {
            Serial.println("No new data received from GPS");
        }
        if (!gps.location.isValid()) {
            Serial.println("GPS location is not valid");
        }
        if (!gps.altitude.isValid()) {
            Serial.println("GPS altitude is not valid");
        }
        if (!gps.hdop.isValid()) {
            Serial.println("GPS HDOP is not valid");
        }
        if (gps.location.age() >= 1000) {
            Serial.println("GPS data is too old");
        }
